"""
instrumented_tabpfn_sri.py
--------------------------
Monkey-patches a fitted TabPFN v2 model (TabPFNV2p6) so we can:
...
Architecture note (TabPFN v2):
  ...
  - Both use separate q/k/v Linear projections + out_projection
  - Internally they call _batched_scaled_dot_product_attention (flash attention),
    which does NOT return attention weights — so we patch forward() to use a
    manual softmax path instead.
