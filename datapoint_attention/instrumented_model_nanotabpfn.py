...
---------------------
A thin wrapper around model.py that exposes:
...
-----
    ...
    base = NanoTabPFNModel(embedding_size=96, num_attention_heads=4,
                           mlp_hidden_size=192, num_layers=3, num_outputs=2)
    base.load_state_dict(torch.load("nanotabpfn_weights.pt", map_location="cpu"))

    model = InstrumentedNanoTabPFN(base)
    model.eval()

    # run with capture
    with torch.no_grad():
        logits, cache = model.forward_with_cache(
            x_train, y_train, train_test_split_index=len(x_train)
        )

    # cache["feature_attn"][layer]    shape: (batch*rows, n_heads, n_cols, n_cols)
    # cache["datapoint_attn"][layer]  shape: (batch*cols, n_heads, n_train+n_test, n_train)
    #   (query dim = n_train for training block, n_test for test block — they are concat'd)
    # cache["activations"][layer]     shape: (batch, rows, cols, embed)

    # head ablation: zero head 2 of the datapoint attention in layer 0
    with torch.no_grad():
        logits_ablated, _ = model.forward_with_cache(
            x_train, y_train, train_test_split_index=len(x_train),
            masked_heads={(0, 2, "datapoint"): True}
        )
"""
...
class InstrumentedTransformerLayer(nn.Module):
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # Helper: reconstruct head-zeroed output from raw attention weights
    # ──────────────────────────────────────────────────────────────────────────
    ...
    def _zero_heads(
        ...
        Strategy: reconstruct V per head, compute head contributions, zero selected,
        then re-project. This avoids re-running the full forward pass.
        """
        if key_src is None:
            key_src = query_src
...
class InstrumentedNanoTabPFN(nn.Module):
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # sklearn-like helpers (for drop-in use in evaluation loops)
    # ──────────────────────────────────────────────────────────────────────────
